...
public class WebExceptionAdvice {
    ...
    public ResponseEntity<String> handler(MethodArgumentTypeMismatchException e) {
        ...
        if (type == null) {
            return ResponseEntity.badRequest().body("请求参数类型不匹配");
        }
        return ResponseEntity.badRequest().body("请求参数类型不匹配");
    }
    ...
    public ResponseEntity<String> handler(MissingServletRequestParameterException e) {
        ...
        return ResponseEntity.badRequest().body("请求参数缺失：" + e.getParameterName());
    }
    ...
    public ResponseEntity<String> handler(HttpRequestMethodNotSupportedException e) {
        ...
        if (supportedMethods == null) {
            ...
            return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body("请求方法错误: 不支持" + method);
        }
        String methods = String.join(", ", supportedMethods);
        log.warn("请求方法错误: 不支持" + method + ", 支持" + methods, e);
        return ResponseEntity.status(HttpStatus.METHOD_NOT_ALLOWED).body("请求方法错误: 不支持" + method + ", 支持" + methods);
    }
    ...
    public ResponseEntity<String> handler(HttpMediaTypeNotSupportedException e) {
        ...
        if (contentType == null) {
            log.warn(e.getMessage(), e);
            return ResponseEntity.status(HttpStatus.UNSUPPORTED_MEDIA_TYPE).body("请求内容类型错误");
        }
    ...
    public ResponseEntity<String> handler(Exception e) {
        log.error(e.getMessage(), e);
        return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body("服务繁忙, 请稍后再试");
    }
